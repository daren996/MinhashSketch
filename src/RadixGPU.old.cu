...
*/
template<int BLOCK_THREADS, int ITEMS_PER_THREAD>
__device__ void BlockGetHashValues (const int k, char *thread_dataDNA, int thread_offset, uint64 *thread_dataR, int hash_b) {
    int list_index = 0;
    ...
    if (k < 32) {
        for (; dna_index < thread_offset + k - 1; ++dna_index) {
            if (thread_dataDNA[dna_index] == 'S')
                isEnd = 1;
            if (base2int(thread_dataDNA[dna_index]) != -1)
                cur_seq[0] = (cur_seq[0] << 2) % ((uint64) 1 << (2 * k)) + base2int(thread_dataDNA[dna_index]);
        }
        ...
        for (; dna_index < thread_offset + k; ++dna_index) {
            ...
            if (base2int(thread_dataDNA[dna_index]) != -1)
                cur_seq[dna_index / 32] =
                        (cur_seq[dna_index / 32] << 2) % UINT64_MAX + base2int(thread_dataDNA[dna_index]);
        }
        if (isEnd)
            thread_dataR[list_index++] = UINT64_MAX;
        else
        ...
        for (; dna_index < thread_offset + ITEMS_PER_THREAD + k - 1; ++dna_index) {
            ...
            if (base2int(thread_dataDNA[dna_index]) != -1)
                cur_seq[k / 32] = (cur_seq[k / 32] << 2) % ((uint64) 1 << (2 * (k % 32))) +
                                  base2int(thread_dataDNA[dna_index]);
            if (isEnd)
                thread_dataR[list_index++] = UINT64_MAX;
            else
                thread_dataR[list_index++] = getHashValue(cur_seq, hash_b, k);
...
/* Get sketch on blocks -
 * Remove duplicate values from sorted hash value list.
*/
template<int BLOCK_THREADS, int ITEMS_PER_THREAD>
__device__ void BlockGetSketch (const int m, int *thread_dataS, int *thread_dataD, uint64 *thread_dataR,
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
        if (thread_dataI[i] < m)
            thread_dataR[thread_dataI[i]] = sketch[thread_dataI[i]];
    }
}
...
__global__ void getBlockSketch (const int k, const int m, char *dna_d, uint64 *input_d,
                                int numElem_dna, int numElem_list, uint64 hash_b) {
    typedef cub::BlockLoad<uint64, BLOCK_THREADS, ITEMS_PER_THREAD, cub::BLOCK_LOAD_TRANSPOSE> BlockLoadR;
    ...
    __shared__ union {
        typename BlockLoadR::TempStorage load;
...
//    BlockLoadR(temp_storage.load).Load(input_d + block_offset, thread_dataR);
//    __syncthreads();
    ...
    __syncthreads();
    BlockGetSketch<BLOCK_THREADS, ITEMS_PER_THREAD>(m, thread_dataS, thread_dataD, thread_dataR, thread_dataI, sketch);
    __syncthreads();
